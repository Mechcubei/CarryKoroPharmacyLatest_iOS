...
class OrderDetailsVC: UIViewController {
    ...
    func getOrderDetailAPI(){
        ...
        NetworkingService.shared.getData_HeaderParameter(PostName: Constants.KCancelOrderDetail, parameters: prams ){ [self] (resp) in
            ...
            let prescription = dataArr!["precription_details"] as! [[String:Any]]
            
            if prescription.count  > 0 {
                let prescrIndex = prescription as!  NSArray
            ...
            for i in 0..<self.medicineStruct.count {
                self.nameArr.append(self.medicineStruct[i].medicine_name)
                self.qtyArr.append(self.medicineStruct[i].quantity)
                self.priceArr.append(self.medicineStruct[i].price)
            }
            
            self.tableHeight.constant = CGFloat(self.nameArr.count * 40)

            self.orderREceivedDate.text =   "Order Received: " + dateConverter(OrderDate: (orderDetail[0]["created_at"] as! String))
            self.txtAddress.text = (orderDetail[0]["address"] as! String)
            self.status.text =  (orderDetail[0]["status"] as! String)
            self.StatusVal =  orderDetail[0]["status"] as! String
            
            let txtOrder = orderDetail[0]["user_instruction"] as! String
            self.txtOrderInstructions.text = txtOrder == "" ? "No Instructions" : txtOrder
    ...
    func dateConverter(OrderDate:String) -> String{
        var convertedLocalTime:String = ""
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd H:mm:ss"
        dateFormatter.timeZone = TimeZone(abbreviation: "UTC")
        if let dt = dateFormatter.date(from: OrderDate) {
