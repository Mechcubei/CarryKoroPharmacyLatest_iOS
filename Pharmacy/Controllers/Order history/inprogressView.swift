...
class inprogressView: UIViewController {
    ...
    @IBOutlet var cancelViewNew: UIView!
    ...
    override func viewDidLoad() {
        ...
        self.StatusVal == "in-process" ? getOrderDetailAPIInprogress()  :  getOrderDetailAPICancel() 
    ...
    func getOrderDetailAPIInprogress(){
        self.showProgress()
        ...
        NetworkingService.shared.getData_HeaderParameter(PostName: Constants.kGetOrderDetail, parameters: prams ){ [self] (resp) in
            ...
            for i in 0..<medicineStruct.count {
                self.nameArr.append(medicineStruct[i].medicine_name)
            }
            ...
            if txtOrder[0] as! String == "" ||  txtOrder[0] as! String == "Enter Prescription"{
                self.orderInstructions.text =  "No instruction"
            }else {
                self.orderInstructions.text = (txtOrder[0] as! String)
            }
            
            let txtAddress = dataArr.value(forKey: "address") as! NSArray
            self.deleveryAddress.text = self.address  //(txtAddress[0] as! String)
    ...
    func getOrderDetailAPICancel(){
        self.showProgress()
        let prams : [String : Any] = ["request_id":requestId]
        NetworkingService.shared.getData_HeaderParameter(PostName: Constants.KCancelOrderDetail, parameters: prams ){ (resp) in
            let res = resp as! NSDictionary
            print(resp)
            let dataArr = (res.value(forKey: "data") as? NSDictionary)
            let orderDetail = dataArr!["order_details"] as! [[String:Any]]

            // prescription code
            let prescr = dataArr!["precription_details"] as! [[String:Any]]
            if prescr.count  > 0 {
                let prescrIndex = prescr[0] as!  NSArray
                self.imgArr = prescrIndex.value(forKey: "pre_image") as? [String] ?? []
                print(self.imgArr.count)
            ...
            self.OrdercollectionView.reloadData()
            // pharmacy detail
            let PharmacyDetails = dataArr!["pharmacy_details"] as! [[String:Any]]
            let med = PharmacyDetails[0]
            let medIndex = med["medicine"] as! [[String:Any]]
            for i in 0..<medIndex.count{
                self.medicineStruct.append(MedicineModel(assigned_pharmacy: "\((medIndex[i] as AnyObject).value(forKey: "assigned_pharmacy")  ?? "")", medicine_id:  "\((medIndex[i] as AnyObject).value(forKey: "id")  ?? "")", medicine_name:  "\((medIndex[i] as AnyObject).value(forKey: "medicine_name")  ?? "")", price: "\((medIndex[i] as AnyObject).value(forKey: "price")  ?? "")", quantity: "\((medIndex[i] as AnyObject).value(forKey: "quantity")  ?? "")",showDeleteButton: "no",showCheck:"no"))
                                
                self.nameArr.append(medIndex[i]["medicine_name"] as! String)
                self.qtyArr.append("\(medIndex[i]["quantity"] as! Int)")
                self.priceArr.append("\(Int( medIndex[i]["price"] as! Double))")
            ...
            let total = (totalValue*100).rounded()/100
            self.totalCost.text = (self.currencies_symbol+"\(total)")
            let txtOrder =  orderDetail[0]["user_instruction"]  as! String
            
            
            if  txtOrder == "Enter Prescription" || txtOrder == ""{
                self.orderInstructions.text =  "No instruction"
            }else{
                self.orderInstructions.text =  txtOrder
            }
                        
            self.deleveryAddress.text = self.address
            self.createdAt.text =  "Order Received: " + (orderDetail[0]["created_at"]  as! String)
            
            let statuus = "cancel"
            self.status.text = statuus //(statuus[0] as! String)
            self.StatusVal =  statuus //(statuus[0] as! String)
            
            if  self.StatusVal == "in-process"{
                self.imgStatus.image = #imageLiteral(resourceName: "inprogress-1")
                self.status.textColor = .blue
                self.cancelViewNew.isHidden = true
            } else if self.StatusVal == "cancel"{
                self.imgStatus.image = #imageLiteral(resourceName: "cancel-1")
                self.status.textColor = .red
                self.blinkingLbl.isHidden = true
                self.cancelViewNew.isHidden = false
                self.lblCancelReason.text = (orderDetail[0]["cancel_reason"]  as! String)
