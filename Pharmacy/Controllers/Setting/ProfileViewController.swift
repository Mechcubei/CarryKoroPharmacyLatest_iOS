...
class ProfileViewController: UIViewController , UIPickerViewDelegate ,URLSessionDataDelegate,URLSessionTaskDelegate,URLSessionDelegate , UINavigationControllerDelegate ,UIImagePickerControllerDelegate{
    ...
    func editPhoneAPI(){
        let params : [String:Any] = ["country_code":"91","phone": txtMobile.text!,
    ...
    func verifyOTPAPI(){
                 var params = [String:Any]()
                     if comingFrom == "email"{
                             params = ["id": id,
                                             "otp":otp,"verification_type":"Email"]
                     }else {
                          params  = ["id": id,
                         "otp":otp,"verification_type":"phone"]
                     }
    ...
    func editProfile(){
           let params : [String:Any] = ["longitude": Long,
                                        "latitude":Lat,"location":txtLocation.text!,"landline":txtLandline.text!]
   ...
   func GetRatingAPI(){
                  ...
                  let res = resp as! NSDictionary
                    ...
                    self.ratingVM.append(RatingModel(pharmacyRating: "\((rating[i] as AnyObject).value(forKey: "") ?? "")", first_name: "\((rating[i] as AnyObject).value(forKey: "first_name") ?? "")",
                                                     id: Int((rating[i] as AnyObject).value(forKey: "id") as! String)!,
    ...
    //MARK:-----------------------------------------Register API --------------------------------------------
           func GetProfile(){
               ...
               if resp.emailStatus == "0" {
                    self.lblChangeEmail.text = "Verify?"
                self.imgValidInvalid.image =  #imageLiteral(resourceName: "cancel")
                  
                }else {
                    self.lblChangeEmail.text = "Update?"
                self.imgValidInvalid.image =  #imageLiteral(resourceName: "verified")                }
                
                if resp.phoneStatus == "0" {
                                   self.lblChangePhone.text = "Verify?"
                    self.editPhoneImg.image =  #imageLiteral(resourceName: "cancel")
                                 
                               }else {
                                   self.lblChangePhone.text = "Update?"
                    self.editPhoneImg.image =  #imageLiteral(resourceName: "verified")
                }
